...
// NX: nombre de la carpeta tal y como se distribuye este fork. Se usa solo como
// último recurso en getExtensionUrl() - ver comentario ahí abajo sobre por qué
// no basta con esta constante si el usuario renombra la carpeta al instalar.
const EXTENSION_DIR = "NX-Character-Hub";
const CL_SETTINGS_KEY = 'SillyTavernCharacterGallery';

// NX: capturado de forma sincrona en tiempo de carga del script (document.currentScript
// solo es fiable durante la ejecucion inicial de un <script> clasico, no dentro de
// callbacks async). Esta es la forma robusta de saber desde donde se cargo ESTE
// archivo, sin depender de que el nombre de la carpeta coincida con ningun texto
// fijo - así sigue funcionando aunque el usuario renombre la carpeta al instalar.
const SELF_SCRIPT_URL = (typeof document !== 'undefined' && document.currentScript && document.currentScript.src) || null;

// Helper to get the correct path for this extension
function getExtensionUrl() {
    // 1) Vía preferida: la URL del propio <script> que cargó este archivo, capturada
    // arriba en tiempo de carga. Es independiente del nombre de la carpeta.
    if (SELF_SCRIPT_URL) {
        return SELF_SCRIPT_URL.substring(0, SELF_SCRIPT_URL.lastIndexOf('/'));
    }
    // 2) Fallback: buscar entre TODOS los <script> de la página uno cuyo src
    // contenga el nombre de carpeta esperado (por si SELF_SCRIPT_URL no estuviera
    // disponible por algún motivo, p.ej. carga vía eval/blob).
    ...
    // 3) Último recurso, ruta fija asumiendo que la carpeta se llama EXTENSION_DIR.
    // Si el usuario instaló con otro nombre de carpeta, esto rompe - por eso los
    // dos métodos de arriba son los que realmente importan.
...
// ==============================================
// Native character panel folder grouping (NX Character Hub)
// ==============================================
// Ported/adapted from CTM's stcm_folders_ui.js pattern: insert our own nav
// panel as the first child of #rm_print_characters_block, never reorder ST's
// own .character_select nodes, and hide the native cards that belong to a
// folder so they don't show twice.
//
// IMPORTANT: this used to live in modules/organize.js, which only runs while
// the "NX Character Hub" tab/popup (library.html) is open — closing it stops
// the script, and ST's next re-render of the character list wipes the
// injected panel. index.js, by contrast, is the extension's main entry point
// and runs continuously in the real ST page for as long as ST itself is
// open, same as CTM's own index.js — so the grouping logic belongs here.
// organize.js still owns the actual folder CRUD (create/rename/assign/etc.);
// this only *reads* the same extensionSettings key to display it.
...
function nxGetFolders() {
    try {
        const ctx = SillyTavern?.getContext?.();
        const v = ctx?.extensionSettings?.[CL_SETTINGS_KEY]?.nxFolders;
        return Array.isArray(v) ? v : [];
    } catch {
        return [];
    }
}
function nxFindFolder(id) {
    return nxGetFolders().find(f => f.id === id) || null;
}
function nxChildrenOf(parentId) {
    return nxGetFolders().filter(f => (f.parentId || null) === (parentId || null));
}
function nxCountRecursive(folderId) {
    const f = nxFindFolder(folderId);
    if (!f) return 0;
    let n = (f.characters || []).length;
    for (const kid of nxChildrenOf(folderId)) n += nxCountRecursive(kid.id);
    return n;
}

let _nxNavFolderId = null;         // null = folder root, else a folder id being browsed
let _nxNativeInjecting = false;
let _nxNativeObserver = null;
let _nxNativeDebounce = null;

function nxExtractAvatarFromSrc(src) {
    if (!src) return null;
    try {
        const parsed = new URL(src, window.location.origin);
        const qp = parsed.searchParams.get('avatar') || parsed.searchParams.get('avatar_url');
        if (qp) return decodeURIComponent(qp.split('/').pop() || qp);
        const m = parsed.pathname.match(/\/characters\/([^/?#]+)/i);
        if (m?.[1]) return decodeURIComponent(m[1]);
    } catch {
        const m = src.match(/\/characters\/([^/?#]+)/i);
        if (m?.[1]) {
...
// Same matching strategy as the display-name override above: numeric
// chid/id first (indexes straight into ctx().characters), then the avatar
// filename parsed out of the card's own image.
function nxAvatarForNativeItem(item, characters) {
    ...
    if (Number.isInteger(numericId) && numericId >= 0 && numericId < characters.length) {
        return characters[numericId]?.avatar || null;
    }
    const img = item.querySelector('img[src]');
    return nxExtractAvatarFromSrc(img?.getAttribute('src') || '');
}

function nxThumbUrl(avatar) {
    return `/thumbnail?type=avatar&file=${encodeURIComponent(avatar)}`;
}

function nxMakeFolderRow(nav, folderId) {
    const f = nxFindFolder(folderId);
    const row = document.createElement('div');
    row.className = 'nx-native-folder-row';
    const count = nxCountRecursive(f.id);
    row.innerHTML = `
        <span class="nx-folder-dot" style="background:${nxEscapeHtml(f.color || 'var(--accent)')}"></span>
        <span class="nx-tree-name">${nxEscapeHtml(f.name)}</span>
        <span class="nx-tree-count">${count}</span>
        <i class="fa-solid fa-chevron-right"></i>
    `;
    row.addEventListener('click', () => {
        _nxNavFolderId = f.id;
        nxRenderNativeNav(nav);
    });
    return row;
}

// Clicking one of our cards should behave exactly like clicking the real
// character in ST's own panel (open it in the native character view), not
// open any modal of our own — so we just replay a click on the real,
// currently-hidden native node for that avatar instead of reimplementing
// character selection ourselves.
function nxMakeCharCard(char, nativeItemsByAvatar) {
    const card = document.createElement('div');
    card.className = 'nx-native-char-card';
    card.title = char.name || '';
    
    // Leer nickname desde extensionSettings de CharNicknames
    let nickname = '';
    try {
        const ctx = window.SillyTavern?.getContext?.();
        if (ctx?.extensionSettings?.nicknames) {
            nickname = ctx.extensionSettings.nicknames[char.name] || '';
        }
    } catch { /* silencioso */ }
    
    // Leer tags del personaje
    let tagsHtml = '';
    try {
        const ctx = window.SillyTavern?.getContext?.();
        if (ctx?.characters && char.name) {
            const charObj = ctx.characters.find(c => c.name === char.name);
            if (charObj?.tags && Array.isArray(charObj.tags) && charObj.tags.length > 0) {
                const tagTexts = charObj.tags.slice(0, 3).map(t => nxEscapeHtml(String(t)));
                tagsHtml = `<span class="nx-native-char-tags">${tagTexts.join(', ')}</span>`;
            }
        }
    } catch { /* silencioso */ }
    
    card.innerHTML = `
        <img src="${nxThumbUrl(char.avatar)}" loading="lazy" alt="">
        <div class="nx-native-char-content">
            <span class="nx-native-char-name">${nxEscapeHtml(char.name || '')}</span>
            ${tagsHtml}
            ${nickname ? `<span class="nx-native-char-nickname">📝 ${nxEscapeHtml(nickname)}</span>` : ''}
        </div>
    `;
    card.addEventListener('click', () => {
        const nativeItem = nativeItemsByAvatar.get(char.avatar);
        if (nativeItem) nativeItem.click();
    });
    return card;
}

function nxRenderNativeNav(nav) {
    const context = SillyTavern?.getContext?.();
    const characters = Array.isArray(context?.characters) ? context.characters : [];
    const nativeItemsByAvatar = nav._nxNativeItemsByAvatar || new Map();

    nav.innerHTML = '';

    if (_nxNavFolderId) {
        const folder = nxFindFolder(_nxNavFolderId);
        if (!folder) { _nxNavFolderId = null; nxRenderNativeNav(nav); return; }

        const header = document.createElement('div');
        header.className = 'nx-native-nav-header';
        header.innerHTML = `
            <i class="fa-solid fa-arrow-left nx-native-nav-back"></i>
            <span class="nx-folder-dot" style="background:${nxEscapeHtml(folder.color || 'var(--accent)')}"></span>
            <span class="nx-tree-name">${nxEscapeHtml(folder.name)}</span>
        `;
        header.querySelector('.nx-native-nav-back').addEventListener('click', () => {
            _nxNavFolderId = folder.parentId || null;
            nxRenderNativeNav(nav);
        });
        nav.appendChild(header);

        const kids = nxChildrenOf(folder.id);
        if (kids.length) {
            const wrap = document.createElement('div');
            wrap.className = 'nx-native-nav-folders';
            for (const kid of kids) wrap.appendChild(nxMakeFolderRow(nav, kid.id));
            nav.appendChild(wrap);
        }

        const chars = (folder.characters || [])
            .map(avatar => characters.find(c => c.avatar === avatar))
            .filter(Boolean);
        if (chars.length) {
            const grid = document.createElement('div');
            grid.className = 'nx-native-nav-grid';
            for (const c of chars) grid.appendChild(nxMakeCharCard(c, nativeItemsByAvatar));
            nav.appendChild(grid);
        }

        if (!kids.length && !chars.length) {
            const empty = document.createElement('div');
            empty.className = 'nx-native-nav-empty';
            empty.textContent = 'Carpeta vacía.';
            nav.appendChild(empty);
        }
    } else {
        const roots = nxChildrenOf(null);
        const label = document.createElement('div');
        label.className = 'nx-native-nav-label';
        label.innerHTML = `<i class="fa-solid fa-folder-tree"></i> Carpetas`;
        nav.appendChild(label);

        const wrap = document.createElement('div');
        wrap.className = 'nx-native-nav-folders';
        for (const f of roots) wrap.appendChild(nxMakeFolderRow(nav, f.id));
        nav.appendChild(wrap);
    }
}

function nxApplyNativeHiding(block, characters, nav) {
    const folderedAvatars = new Set();
    for (const f of nxGetFolders()) {
        for (const a of (f.characters || [])) folderedAvatars.add(a);
    }
    const nativeItemsByAvatar = new Map();
    const items = block.querySelectorAll(':scope > .character_select');
    for (const item of items) {
        const avatar = nxAvatarForNativeItem(item, characters);
        if (avatar) nativeItemsByAvatar.set(avatar, item);
        if (avatar && folderedAvatars.has(avatar)) {
            item.classList.add('nx-native-hidden');
        } else {
            item.classList.remove('nx-native-hidden');
        }
    }
    nav._nxNativeItemsByAvatar = nativeItemsByAvatar;
}

function nxEnsureNativeStyles() {
    if (document.getElementById('nx-native-folder-styles')) return;
    const style = document.createElement('style');
    style.id = 'nx-native-folder-styles';
    style.textContent = `
        #rm_print_characters_block .nx-native-hidden { display: none !important; }
        .nx-native-folder-nav {
            display: block; width: 100%; margin: 2px 0 10px 0;
        ...
        .nx-native-nav-label {
            ...
            padding: 4px 6px 8px 6px; font-weight: 600; opacity: 0.85;
        }
        .nx-native-nav-header {
            display: flex; align-items: center; gap: 8px;
            padding: 6px; margin-bottom: 4px; font-weight: 600;
            border-bottom: 1px solid rgba(255,255,255,0.08);
        }
        .nx-native-nav-back {
            cursor: pointer; padding: 2px 4px; border-radius: 6px;
            color: var(--SmartThemeQuoteColor, #a0a0a0);
        }
        .nx-native-nav-back:hover { background: rgba(255,255,255,0.08); }
        .nx-native-nav-folders { display: flex; flex-direction: column; gap: 2px; }
        .nx-native-folder-row {
            display: flex; align-items: center; gap: 6px;
            padding: 7px 8px; border-radius: 8px; cursor: pointer; user-select: none;
        }
        .nx-native-folder-row:hover { background: rgba(255,255,255,0.06); }
        .nx-native-folder-row i.fa-chevron-right {
            font-size: 0.75em; color: var(--SmartThemeQuoteColor, #a0a0a0);
        }
        .nx-native-nav-grid {
        ...
        .nx-native-char-card {
            ...
            padding: 8px 8px; border-radius: 8px; box-sizing: border-box;
        }
        .nx-native-char-card:hover { background: rgba(255,255,255,0.06); }
        .nx-native-char-card img {
            width: 60px; height: 90px; flex-shrink: 0;
            object-fit: cover; border-radius: 6px;
            border: 1px solid rgba(255,255,255,0.1);
        }
        .nx-native-char-content {
            display: flex; flex-direction: column; gap: 4px;
            flex: 1; min-width: 0;
        }
        .nx-native-char-card .nx-native-char-name {
            font-size: 0.95em; font-weight: 600; line-height: 1.2;
            overflow: hidden; text-overflow: ellipsis; white-space: nowrap;
        }
        .nx-native-char-card .nx-native-char-tags {
            font-size: 0.78em; opacity: 0.7; line-height: 1.1;
            overflow: hidden; text-overflow: ellipsis;
            display: -webkit-box; -webkit-line-clamp: 2; -webkit-box-orient: vertical;
        }
        .nx-native-char-card .nx-native-char-nickname {
            font-size: 0.78em; opacity: 0.85; line-height: 1.1;
            overflow: hidden; text-overflow: ellipsis; white-space: nowrap;
        }
        .nx-native-nav-empty { padding: 10px 6px; opacity: 0.6; font-style: italic; }
        .nx-native-folder-nav .nx-folder-dot {
            width: 10px; height: 10px; border-radius: 3px; flex-shrink: 0;
            border: 1px solid rgba(255, 255, 255, 0.25);
        }
        .nx-native-folder-nav .nx-tree-name {
            flex: 1; min-width: 0; overflow: hidden;
            text-overflow: ellipsis; white-space: nowrap;
        }
        .nx-native-folder-nav .nx-tree-count {
            color: var(--SmartThemeQuoteColor, #a0a0a0); font-size: 0.85em;
...
function nxInjectNativePanel() {
    try {
        const block = document.getElementById('rm_print_characters_block');
        if (!block) return;
        if (_nxNativeInjecting) return;

        nxEnsureNativeStyles();

        const folders = nxGetFolders();
        let nav = block.querySelector(':scope > .nx-native-folder-nav');

        if (!folders.length) {
            if (nav) nav.remove();
            block.querySelectorAll(':scope > .character_select.nx-native-hidden')
                .forEach(el => el.classList.remove('nx-native-hidden'));
            return;
        }

        _nxNativeInjecting = true;
        try {
            if (!nav) {
                nav = document.createElement('div');
                nav.className = 'nx-native-folder-nav';
                block.insertBefore(nav, block.firstChild);
            } else if (nav !== block.firstChild) {
                block.insertBefore(nav, block.firstChild);
            }

            const context = SillyTavern?.getContext?.();
            const characters = Array.isArray(context?.characters) ? context.characters : [];
            nxApplyNativeHiding(block, characters, nav);
            nxRenderNativeNav(nav);
        } finally {
            _nxNativeInjecting = false;
        }
    } catch (e) {
        console.error('[NX Folders] nxInjectNativePanel error:', e);
    }
}

function nxRefreshNativePanel() {
    clearTimeout(_nxNativeDebounce);
    _nxNativeDebounce = setTimeout(nxInjectNativePanel, 60);
}

// Folders can also be created/renamed/assigned from the "Carpetas" tab, which
// runs in a separate popup/tab (library.html) and only touches
// extensionSettings — it doesn't cause any DOM mutation of
// #rm_print_characters_block for our MutationObserver to catch. Poll for
// changes to the stored folder data so those edits reach this panel without
// requiring a manual ST reload.
let _nxLastFoldersJson = '';
function nxPollFolders() {
    try {
        const json = JSON.stringify(nxGetFolders());
        if (json !== _nxLastFoldersJson) {
            _nxLastFoldersJson = json;
            nxRefreshNativePanel();
        }
    } catch { /* ignore, next poll will retry */ }
}

function initNativeFolderNav() {
    const block = document.getElementById('rm_print_characters_block');
    if (!block) {
        setTimeout(initNativeFolderNav, 1000);
        return;
    }
    if (_nxNativeObserver) return;
    _nxNativeObserver = new MutationObserver(() => {
        if (_nxNativeInjecting) return;
        nxRefreshNativePanel();
    });
    _nxNativeObserver.observe(block, { childList: true });
    _nxLastFoldersJson = JSON.stringify(nxGetFolders());
    setInterval(nxPollFolders, 1500);
    nxRefreshNativePanel();
}

...
(function MobileEnhancements() {
    ...
    async function triggerViewRefresh() {
        ...
        if (view === 'online') {
            window.OrganizeModule?.activate?.();
    ...
    function updateFab(view) {
        ...
        switch (view) {
            ...
            case 'chats':
                ...
                break;
            case 'online':
                // NX: "Carpetas" tab has its own toolbar search; no FAB action needed here.
    ...
    function createSettingsButton(topbar) {
        ...
        btn.addEventListener('click', () => {
            ...
            if (activeView === 'online' && (window.ProviderRegistry?.getViewProviders?.() || []).length > 0) {
                const reg = window.ProviderRegistry;
    ...
    function createMenuButton(topbar) {
        ...
        btn.addEventListener('click', () => {
            const hasProviders = (window.ProviderRegistry?.getViewProviders?.() || []).length > 0;
            const isOnline = getActiveView() === 'online' && hasProviders;
    ...
    function createProviderQuickSwitch(topbar) {
        ...
        function updateVisibility() {
            // NX: 'online' is now the "Carpetas" tab, not a provider browser — this
            // quick-switch button only makes sense when providers actually exist.
            const hasProviders = (window.ProviderRegistry?.getViewProviders?.() || []).length > 0;
            const isOnline = getActiveView() === 'online' && hasProviders;
